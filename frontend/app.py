"""
HR Policy Chatbot — Streamlit Frontend
...
if "messages"    not in st.session_state:
    st.session_state.messages    = []      # [{role, content, sources, ts}]
...
def clear_backend_session():
    try:
        requests.post(
            f"{API_BASE}/clear/{st.session_state.session_id}", timeout=5
        )
    except Exception:
        pass
...
with chat_placeholder:
    ...
    # Typing indicator + actual API call
    if st.session_state.thinking:
        ...
        # Get the last user message and call API
        last_user = next(
        ...
        if last_user:
            try:
                ...
                st.session_state.messages.append({
                    "role":    "bot",
                    "content": "⚠️ **Cannot connect to the backend.** "
                               "Please start the FastAPI server (`python main.py`) and refresh.",
                    "sources": [],
                    "ts":      time.strftime("%H:%M"),
                })
...
# Handle Enter key (detect by checking input change)
